...
txt1 = tkinter.Entry(bg='white', font='verdana 12')
txt1.grid(row=0, column=1, sticky='w')

txt2 = tkinter.Entry(bg='white', font='verdana 12')
txt2.grid(row=1, column=1, sticky='w')

# Label to show result
result_lbl = tkinter.Label(text="Result will appear here", bg='lightblue', fg='darkgreen', font='verdana 14 bold')
result_lbl.grid(row=2, column=0, columnspan=2, pady=10)

def btnclick1():
    no1 = txt1.get()
    no2 = txt2.get()
    no3 = int(no1) + int(no2)
    result_lbl.config(text=f"Answer: {no3}")
...
def btnclick4():
    ...
    if int(no2) != 0:
        no3 = int(no1) / int(no2)
        result_lbl.config(text=f"Answer: {no3:.2f}")
    else:
        result_lbl.config(text="Error: Cannot divide by 0")
